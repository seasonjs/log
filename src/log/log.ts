...
import {Emitter, Event} from '../base';
import {hash} from '../base';
import {Disposable, IDisposable} from '../base';
import {isWindows} from '../base';
import {Mutable, isNumber, isString} from '../base';
...
export abstract class AbstractLoggerService extends Disposable implements ILoggerService {
  ...
  private readonly _loggers = new Map<string | URI, LoggerEntry>();
  ...
  protected toResource(idOrResource: string | URI): URI {
    return isString(idOrResource) ? URI.joinPath(this.logsHome, `${idOrResource}.log`) : idOrResource;
  }
